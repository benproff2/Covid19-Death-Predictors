...
df_display = pd.DataFrame({
    ...
    "No-Class Metrics": df_metrics[["Precision_No", "Recall_No", "F1_No"]]
        .apply(lambda row: f"Precision = {row.iloc[0]:.3f}\nRecall = {row.iloc[1]:.3f}\nF1 Score = {row.iloc[2]:.3f}", axis=1),
    "Yes-Class Metrics": df_metrics[["Precision_Yes", "Recall_Yes", "F1_Yes"]]
        .apply(lambda row: f"Precision = {row.iloc[0]:.3f}\nRecall = {row.iloc[1]:.3f}\nF1 Score = {row.iloc[2]:.3f}", axis=1)
})
...
for i, ax in enumerate(axs):
    ...
    ax.set_title(model_names[i], fontsize=9)
    ax.set_xticks([0, 1])
    ax.set_yticks([0, 1])
...
plt.tight_layout()
plt.savefig("myplot_confusion.png", dpi=300, bbox_inches="tight")
plt.show()
